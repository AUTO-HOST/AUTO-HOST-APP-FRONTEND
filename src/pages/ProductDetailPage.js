...
// Importaciones de Firestore para el carrito
import { db } from '../firebaseConfig'; // Asegúrate de que 'db' esté exportado desde firebaseConfig
...
const ProductDetailPage = () => {
  ...
  // --- FUNCIÓN handleBuyNow (AHORA AÑADE AL CARRITO EN FIRESTORE) ---
  const handleBuyNow = async () => {
    ...
    const cartRef = doc(db, "carts", userId); // Referencia al documento del carrito del usuario
    const cartItemRef = collection(cartRef, "items"); // Subcolección de ítems en el carrito

    try {
      // Intentar obtener el documento del carrito para ver si ya existe
      ...
      // Añadir o actualizar el ítem en la subcolección 'items' del carrito
      const itemInCartRef = doc(cartItemRef, product._id); // El ID del documento del ítem será el ID del producto
      ...
      if (itemInCartSnap.exists()) {
        ...
        await updateDoc(itemInCartRef, {
          quantity: increment(1), // Incrementa la cantidad en 1
          totalPrice: increment(itemPrice), // Incrementa el precio total del ítem
          updatedAt: new Date()
        });
        ...
        await setDoc(itemInCartRef, {
          ...
          sellerId: product.user, // ID del vendedor
